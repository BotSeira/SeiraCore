...
public final class DcsCommandHandler {
    ...
    public void handleDcs(Context ctx) {
        if (!ctx.inGroup()) {
            ctx.sendReply(PendingMessage.ofMarkdownRaw(at(ctx) + "/dcs 仅支持在群聊中使用。"));
            return;
        }
    ...
    private void handleStart(Context ctx) {
        ...
        if (target == null) {
            ctx.sendReply(PendingMessage.ofMarkdownRaw(at(ctx) + "Discord 目标格式无效，应为 <guild-id>.<channel-id>。"));
            return;
        }
        ...
        if (!result.success()) {
            ctx.sendReply(PendingMessage.ofMarkdownRaw(at(ctx) + "开启 Discord 同步失败：" + result.message()));
            return;
        }
    ...
    private void handleStop(Context ctx) {
        ...
        ctx.sendReply(PendingMessage.ofMarkdownRaw(at(ctx) + (removed ? "Discord 消息同步已解除。" : "当前群聊尚未开启 Discord 消息同步。")));
    }
}
