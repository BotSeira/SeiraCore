...
上传 osu! 服务器上不存在的 `.osr` 回放后，机器人会返回形如 `loc123456789` 的本地成绩ID；该ID可用于 `/s`、`/sa`、`/ma`、`/r`、
`/rsc` 等成绩目标指令。
...
这些指令会共享最近一次显式指定的查询目标。已有最近目标时，`/r`、`/rsc`、`/ma` 可以省略目标，例如 `/r 01:00-01:30`、
`/rsc +12345,67890 -`、`/ma #2`。
