...
目标记忆按指令调用者独立保存，记录当前目标关联的谱面集、具体谱面和成绩 ID。省略目标时，会优先使用所需类型的记忆，缺少时按需查找并缓存：

- `/m 12345` 后使用 `/ms`、`/dl` 或 `/ap`，会查找该谱面所属的谱面集；使用 `/s` 会查找调用者绑定账号在该谱面上的成绩。
- `/s 成绩ID` 后使用 `/m`、`/bma` 或 `/ms`，可反查该成绩对应的谱面或谱面集。也支持本地成绩 ID。
- `/s @用户` 会使用记忆中的谱面查找该用户的成绩；没有记忆时提示用法。
- `/rs`、`/rp`、`/bp` 的单条成绩查询及其快捷查询保存实际成绩 ID；之后 `/sa`、`/ma`、`/r` 使用同一条成绩。
- `/bpv` 可省略谱面目标，`/bpv +HD` 可为记忆的谱面指定 Mods。

只有谱面集记忆时，不自动选择难度；请使用 `/m 谱面集ID#难度序号` 或指定谱面 ID。切换到新的目标会清除旧目标的关联信息，补全查找失败保留原有记忆。记忆保存在运行进程中，重启后清空。
...
| `/bp`           | `/bp [n] [uid/username/@user] [filters...]`             | 获取最多200条最好成绩后过滤；省略n时返回第一个匹配成绩详情                      |
...
这些指令会共享最近一次显式指定的查询目标。已有最近目标时，`/r`、`/rsc`、`/ma` 可以省略目标，例如 `/r 01:00-01:30`、`/rsc +12345,67890 -`、`/ma #2`。

也可以在前面写上玩家ID、用户名或@用户，例如 `123456 rs5`、`peppy bp3`、`@ABC bp3`，表示查询指定玩家的最近成绩第 5 条或最好成绩第 3 条。
